...
/// Build the shared [`Opts`] for a diagnostic fetch. Checks default off; only
/// `verify` re-enables them, matching the suite's debug-first posture.
...
/// Drive one fetch and print the answering server, byte length, elapsed time,
/// and the response body (pretty-printed JSON, or lossy UTF-8 on parse
/// failure).
